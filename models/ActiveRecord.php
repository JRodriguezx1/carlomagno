...
class ActiveRecord {
    ...
    public static function eliminar_idregistros($id, $array = []){
       ...
       foreach($array as $key => $value){
        ...
        }   //$sqldelete = DELETE FROM tabla WHERE Id IN (1140866000, 63502720);
        $resultado = self::$db->query($sqldelete);
        return $resultado;
    }
    ...
    public function actualizar(){
        ...
        return $resultado;
              ...
              //actualizar multiples registros (valores, ids)
    ...
    public static function numregistros(){
        $sql = "SELECT COUNT(*) FROM ".static::$tabla.";"; //COUNT(*) contar todos los registros de una tabla
        $resultado = self::$db->query($sql);
    ...
    public static function whereArray($array = []){ ////
        ...
        foreach($array as $key => $value){
            if(array_key_last($array) == $key){  //toma la ultima llave y la compara con la llave actual
                $sql.= " ${key} = '${value}'";
            }else{
                $sql.= " ${key} = '${value}' AND ";
            }
